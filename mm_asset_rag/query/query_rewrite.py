...
This module layers on top of :func:`mm_asset_rag.query.retrieval.hybrid_search` —
...
2. :func:`multi_query_search` — fan out :func:`hybrid_search` over
   ...
   :func:`mm_asset_rag.query.retrieval.merge_hits` (rank-based RRF).
3. :func:`hybrid_search_with_rewrite` — top-level wrapper. ``query_rewrite_enabled=True``
   ...
   :func:`mm_asset_rag.query.retrieval.hybrid_search`.
...
- The LLM call is **best-effort**. A rewrite failure (timeout / no
  ...
  mirrors :func:`mm_asset_rag.answer.answer.fallback_answer` and
  :func:`mm_asset_rag.ingest.auto_meta.auto_meta_image`.
- Image routes (``text-to-image`` / ``image-to-image``) skip rewrite
  ...
  in :func:`mm_asset_rag.query.search_service.dispatch_search`, not here.
...
- The rewrite HTTP call is its own private helper
  ...
  :func:`mm_asset_rag.answer.answer._post_chat` — because rewrite only
...
def _strip_think_tags(content: str) -> str:
    ...
    The LLM channel in :func:`mm_asset_rag.answer.answer.llm_answer` strips
...
def hybrid_search_with_rewrite(
    ...
    *,
    ...
    transparent pass-through to :func:`mm_asset_rag.query.retrieval.hybrid_search`
...
def _multi_query_text(
    ...
    *,
    ...
    what :func:`mm_asset_rag.query.search_service.dispatch_search` routes
