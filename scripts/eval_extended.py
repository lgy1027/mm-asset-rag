...
EVAL_CASES: list[dict] = [
    ...
    # keyword
    {"query": "Mona Lisa painting",                "expected_ids": ["wiki_en_monalisa"],                          "category": "wiki_en_kw",   "mode": "text"},
    {"query": "Bicycle history",                   "expected_ids": ["wiki_en_bicycle"],                           "category": "wiki_en_kw",   "mode": "text"},
    {"query": "Pizza origin Italy",                "expected_ids": ["wiki_en_pizza"],                             "category": "wiki_en_kw",   "mode": "text"},
    {"query": "Coca-Cola brand",                   "expected_ids": ["wiki_en_cocacola"],                          "category": "wiki_en_kw",   "mode": "text"},
    {"query": "Piano musical instrument",          "expected_ids": ["wiki_en_piano"],                             "category": "wiki_en_kw",   "mode": "text"},
    {"query": "Mushroom biology",                  "expected_ids": ["wiki_en_mushroom"],                          "category": "wiki_en_kw",   "mode": "text"},
    {"query": "Telescope astronomy",               "expected_ids": ["wiki_en_telescope"],                         "category": "wiki_en_kw",   "mode": "text"},
    {"query": "Photography",                       "expected_ids": ["wiki_en_photography"],                       "category": "wiki_en_kw",   "mode": "text"},
    {"query": "QR code two-dimensional barcode",   "expected_ids": ["wiki_en_qrcode"],                            "category": "wiki_en_kw",   "mode": "text"},
    {"query": "PDF file format Adobe",             "expected_ids": ["wiki_en_pdf"],                               "category": "wiki_en_kw",   "mode": "text"},
    {"query": "Apollo 11 moon landing",            "expected_ids": ["wiki_en_apollo11"],                          "category": "wiki_en_kw",   "mode": "text"},
    {"query": "Sony corporation history",          "expected_ids": ["wiki_en_sony"],                              "category": "wiki_en_kw",   "mode": "text"},
    # semantic paraphrase — should still hit via dense
    {"query": "renaissance portrait with enigmatic smile",
                                                "expected_ids": ["wiki_en_monalisa"],                          "category": "wiki_en_sm",   "mode": "text"},
    {"query": "human-powered two-wheeled transport",
                                                "expected_ids": ["wiki_en_bicycle"],                           "category": "wiki_en_sm",   "mode": "text"},
    {"query": "Italian flatbread topped with tomato and cheese",
                                                "expected_ids": ["wiki_en_pizza"],                             "category": "wiki_en_sm",   "mode": "text"},
    {"query": "Atlanta-based soft drink company",
                                                "expected_ids": ["wiki_en_cocacola"],                          "category": "wiki_en_sm",   "mode": "text"},
    {"query": "88-key hammer-string instrument",
                                                "expected_ids": ["wiki_en_piano"],                             "category": "wiki_en_sm",   "mode": "text"},
    {"query": "fruiting body of fungus",
                                                "expected_ids": ["wiki_en_mushroom"],                          "category": "wiki_en_sm",   "mode": "text"},
    {"query": "NASA's first crewed lunar landing mission",
                                                "expected_ids": ["wiki_en_apollo11"],                          "category": "wiki_en_sm",   "mode": "text"},
    {"query": "Japanese electronics conglomerate founded post-war",
                                                "expected_ids": ["wiki_en_sony"],                              "category": "wiki_en_sm",   "mode": "text"},

    # ── Wikipedia ZH (4 assets) ────────────────────────────────────────────
    {"query": "北京 中国首都",                          "expected_ids": ["wiki_zh_beijing"],                      "category": "wiki_zh_kw",   "mode": "text"},
    {"query": "肯德基 KFC 快餐",                         "expected_ids": ["wiki_zh_kfc"],                           "category": "wiki_zh_kw",   "mode": "text"},
    {"query": "咖啡 饮品",                              "expected_ids": ["wiki_zh_coffee"],                        "category": "wiki_zh_kw",   "mode": "text"},
    {"query": "大熊猫 保护动物",                         "expected_ids": ["wiki_zh_panda"],                         "category": "wiki_zh_kw",   "mode": "text"},
    # Chinese semantic
    {"query": "中国的政治中心和文化古城",                  "expected_ids": ["wiki_zh_beijing"],                      "category": "wiki_zh_sm",   "mode": "text"},
    {"query": "源自美国的炸鸡连锁餐厅",                   "expected_ids": ["wiki_zh_kfc"],                           "category": "wiki_zh_sm",   "mode": "text"},
    {"query": "含咖啡因的热带植物种子饮品",                "expected_ids": ["wiki_zh_coffee"],                        "category": "wiki_zh_sm",   "mode": "text"},
    {"query": "中国特有的黑白熊科动物",                    "expected_ids": ["wiki_zh_panda"],                         "category": "wiki_zh_sm",   "mode": "text"},

    # ── arXiv (2 assets) ───────────────────────────────────────────────────
    {"query": "bolometer thermodynamics economics",        "expected_ids": ["arxiv_phys_thermo"],                "category": "arxiv_kw",    "mode": "text"},
    {"query": "spiking neuron soliton model",             "expected_ids": ["arxiv_qbio_neuro"],                 "category": "arxiv_kw",    "mode": "text"},
    # semantic
    {"query": "non-ML cross-disciplinary physics paper about money",
                                                        "expected_ids": ["arxiv_phys_thermo"],                "category": "arxiv_sm",    "mode": "text"},
    {"query": "biological neuron firing dynamics paper",
                                                        "expected_ids": ["arxiv_qbio_neuro"],                 "category": "arxiv_sm",    "mode": "text"},

    # ── IRS forms (2 assets) ───────────────────────────────────────────────
    {"query": "W-9 taxpayer identification request",      "expected_ids": ["irs_w9"],                           "category": "irs_kw",     "mode": "text"},
    {"query": "W-4 employee withholding certificate",     "expected_ids": ["irs_w4"],                           "category": "irs_kw",     "mode": "text"},
    # semantic
    {"query": "US tax form a contractor fills out for a client",
                                                        "expected_ids": ["irs_w9"],                           "category": "irs_sm",     "mode": "text"},
    {"query": "US form employees submit to control payroll tax withholding",
                                                        "expected_ids": ["irs_w4"],                           "category": "irs_sm",     "mode": "text"},
    ...
    {"query": "scanned Mona Lisa article",               "expected_ids": ["wiki_en_monalisa__scan"],            "category": "scan_kw",    "mode": "text"},
    {"query": "scanned pizza article",                   "expected_ids": ["wiki_en_pizza__scan"],               "category": "scan_kw",    "mode": "text"},
    {"query": "OCR-output page about Leonardo da Vinci portrait",
                                                        "expected_ids": ["wiki_en_monalisa__scan"],            "category": "scan_sm",    "mode": "text"},
]
...
def render_report(report: dict, results: list[dict]) -> str:
    ...
    for cat, m in report["per_category"].items():
        lines.append(
            f"    {cat:18s}  hit_rate={m['hit_rate']:.3f}   "
