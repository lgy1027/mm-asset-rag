...
### Added
- **可插拔全链路 tracing(Langfuse)**:新增 provider 中立的 `Tracer`/`Span` 协议(`mm_asset_rag/core/observability.py`),默认 `NoOpTracer` 零开销;`TRACING_PROVIDER=langfuse` + 可选 `[langfuse]` extra(langfuse v3 SDK,lazy import,未安装时静默降级 no-op)接入自托管 Langfuse。插桩覆盖:`llm.chat_completion`(全部 LLM 调用,含 rewrite/answer/VLM/contextual)、`search.dispatch`、`qdrant.text_search`、`rerank.score`、`answer.generate`、`ingest.task`,自动记录 latency,异常自动标 ERROR。凭据缺失/SDK 未装均只告警不报错。并行 multi-query rewrite 的 ThreadPoolExecutor 子 span 通过 `opentelemetry-instrumentation-threading` 正确嵌套进调用方 trace(否则会成为孤儿 root trace)。新增 `scripts/docker-compose.langfuse.yml`(langfuse-web + worker + postgres + redis + clickhouse + minio 全栈自托管,`LANGFUSE_INIT_*` 首次启动自动建项目与 API key)。零行为变化:不设 `TRACING_PROVIDER` 时与之前完全一致。
- **tracing 分段补全**:新增 `embed.text`(远程文本 embedding)、`embed.image_text`/`embed.image`(CLIP 编码)span;qdrant/rerank/LLM 各段补齐响应参数——检索 top 分数与 asset、rerank blend 与重排分数、非流式 LLM 的 usage token 计数与截断输出文本。单条 `search.dispatch` trace 现在覆盖 rewrite → embed → qdrant → rerank 全链路,每段耗时与响应参数可在 Langfuse 逐层查看。
### Changed
...
### Changed
- Harden multimodal ingestion, retrieval, evaluation, documentation, and CI coverage.

## [0.2.1] - 2026-08-19

### Added
- **中文 README** (`README.zh-CN.md`) — English / 中文 双向链接,沿用同一结构与代码示例,补 `[cn_clip]` / `[ocr]` / `[docling]` extra 装包说明。
...
- `.env.example` `IMAGE_PROVIDER=cn_clip` 注释补全:三种装 `transformers` 路径(`[cn_clip]` extra / `[docling]` 传递依赖 / 单独 `pip install transformers`),不依赖默认的 `[docling]` 间接带入。
- `pyproject.toml` `[cn_clip]` extra 的注释说明它**是**裸装环境(只想要中文 CLIP)的 canonical 入口,不依赖 `[docling]` 间接拉入 `transformers`。
...
- **v4 0.2.0: 5 阶段全量优化 (代码 ready,reindex 需手动)**:
  ...
  - **P3 per-channel RRF 权重(原生 RrfQuery)** — 替换 0.2.0 P3 早期的 `_channel_limit()` workaround。改用 Qdrant server 1.17+ 原生 `models.RrfQuery(rrf=models.Rrf(weights=[...], k=60))`,权重数组按 prefetch 顺序[dense, bm25, bm25_zh]位置传。当所有 weight=1.0 时 fallback 到 `FusionQuery(Fusion.RRF)`(uniform 路径,等效原行为);非 1.0 时走 RrfQuery。`RRF_K=60` 暴露为常量。  
  来源:Qdrant 官方文档 https://qdrant.tech/documentation/concepts/hybrid-queries/ (1.17+ weighted RRF)。0.2.0 早期猜测的 `Prefetch.weight=...` 实际不存在,正确位置是 `RrfQuery.rrf.weights`。
- **0.3.0 auto-eval 集成 CI** — `run_text_to_text_eval_v2` / `run_text_to_image_eval_v2` 新增 ``search_fn`` + ``full_ids`` DI hook,支持 mock corpus / mock retriever 跑测试。新 `tests/unit/test_evaluation_v2_regression.py` 跑 6 个 mock-based 测(0.45s vs 完整 v2 eval 几分钟):golden query 在 per-group hit_rate floor(0.50) 上验证 retriever 改动不破坏 `_match` / `_expand` / `_title_of` 的核心契约;negative 测 verify over-recall 容忍;signature test guard DI 钩子不被无意改掉。改动 retriever code → 立刻 CI fail if hit_rate 退化。  
  来源:pytest 文档 https://docs.pytest.org/en/stable/how-to/fixtures.html(`monkeypatch.setattr` autouse fixture + parametrize),DI 模式(caller 注入 search 函数)做 fallback。
